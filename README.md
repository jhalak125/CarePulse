...
An enterprise-grade, full-stack healthcare appointment and follow-up management platform featuring role-based portals for **Patients**, **Doctors**, and **Admins**, powered by **AI clinical pre-visit triage & post-visit care plans**, **atomic slot booking with 10-minute hold locks**, **doctor leave conflict resolution**, **Google Calendar OAuth 2.0 sync**, and **automated background medication & email notification queues**.
...
- **Double-Booking Prevention & 10-Minute Slot Hold Engine**:
  - Prevents race conditions using transactional database locks (`prisma.$transaction`) and ephemeral `SlotHold` reservations (`expiresAt = Now() + 10 mins`).
...
- **LLM Clinical Engine & Fail-Safe Architecture**:
  - Google Gemini API integration with prompt-engineered structured JSON outputs.
...
- **Dual Email Engine & Ethereal Live Web Previews**:
  - Integrated Nodemailer supporting production SMTP / SendGrid alongside automatic Ethereal test accounts (generates clickable web preview URLs for every email in console and admin audit logs).
...
- **Automated Background Workers (`node-cron`)**:
  - Expired slot hold release worker (1 min)
...
# Generate Prisma Client, push database schema (SQLite dev.db), and seed sample data
npm run setup
```
...
| :--- | :--- | :--- | :--- |
| **Patient** | `patient@carepulse.demo` | `Password123!` | Book slots, 10m hold timer, AI symptom triage, medication checklist |
| **Doctor** | `doctor@carepulse.demo` | `Password123!` | View queue, AI briefs, 3 clinical questions, submit post-visit care plans |
| **Admin** | `admin@carepulse.demo` | `Password123!` | KPI dashboard, doctor management, leave conflict audit, email previews |
...
SMTP_USER=
SMTP_PASS=
SMTP_FROM="CarePulse Healthcare <no-reply@carepulse.health>"
```
...
CarePulse utilizes Google Gemini API (`gemini-1.5-flash`) for clinical summaries.
